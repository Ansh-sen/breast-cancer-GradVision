...
# Lazy TensorFlow import — keeps the module importable even if TF is not installed.
# TF is only loaded when a GradCAM function is actually called at runtime.
_tf = None
...
def detect_tumor_region(original_pil_img, heatmap, threshold=0.6, min_area=50, prob=None):
    """
    ...
    try:
        ...
        # Draw bounding box on a copy of the blended heatmap overlay
        tumor_box_img = heatmap_overlay.copy() # Blended Image
        has_tumor = False
        ...
        if largest_contour is not None:
            ...
            cv2.rectangle(tumor_box_img, (x, y), (x + w, y + h), (255, 0, 0), 2) # thin box looks sleek
            
            # Draw contour (optional, green for visibility)
            cv2.drawContours(tumor_box_img, [largest_contour], 0, (0, 255, 0), 1)
            
            # Add label with probability text overlay if passed
            if prob is not None:
                text = f"Prob: {prob:.2f}"
                (tw, th), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.4, 1)
                # Draw black filled rectangle background for text box
                cv2.rectangle(tumor_box_img, (x + w - tw - 6, y + 2), (x + w - 2, y + th + 10), (0, 0, 0), -1)
                # Print White Text
                cv2.putText(tumor_box_img, text, (x + w - tw - 4, y + th + 4), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
            else:
                # Fallback to coordinates
                cv2.putText(tumor_box_img, f"ROI: ({x},{y})", (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 0, 0), 1)
