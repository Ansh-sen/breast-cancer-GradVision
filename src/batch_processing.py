...
class BatchAnalyzer:
    ...
    def analyze_single_image(self, uploaded_file, file_name: str) -> Dict:
        ...
        try:
            ...
            # Detect Tumor Region
            from src.gradcam_utils import detect_tumor_region
            from src.morphology_analysis import analyze_cell_morphology
            
            tumor_result = None
            try:
                tumor_result = detect_tumor_region(pil_img, heatmap, threshold=0.6, prob=raw_pred)
            except Exception:
                pass
    ...
    def analyze_batch(self, uploaded_files: List) -> Dict:
        ...
        for uploaded_file in uploaded_files:
            file_name = uploaded_file if isinstance(uploaded_file, str) else getattr(uploaded_file, "name", "unknown")
            result = self.analyze_single_image(uploaded_file, file_name)
